...
def distill_prerank(
    ...
    log.info(
        "distill pre-ranker: params=%s (teacher mimic)",
    ...
    for ep in range(1, epochs + 1):
        ...
        for s in range(0, n_users, batch_users):
            ...
            with torch.no_grad():
                t_logit = teacher(cand_flat, seq_b, content, cat)[:, 0]
            s_logit = student(cand_flat, seq_b, content, cat)[:, 0]
...
def _consistency(
    ...
    for s in range(0, len(users), 512):
        ...
        cand_flat = cand_t[torch.as_tensor(ub, device=device)].reshape(-1)
        seq_b = seq_t[torch.as_tensor(np.repeat(ub, k), device=device)]
        ts = teacher(cand_flat, seq_b, content, cat)[:, 0].reshape(len(ub), k).cpu().numpy()
        ss = student(cand_flat, seq_b, content, cat)[:, 0].reshape(len(ub), k).cpu().numpy()
